...
<div class="container" style="height: auto;">
  <div class="row align-items-center">
    <div class="col-md-9 ml-auto mr-auto mb-3 text-center">
      <h3>{{ __('Ingresa o registrate a la plataforma de Monitoreo Ciudadano del agua.') }} </h3>
    </div>
          ...
          <div class="card-body">
            ...
            <div class="bmd-form-group{{ $errors->has('email') ? ' has-danger' : '' }}">
              <div class="input-group">
                ...
                <input type="email" name="email" class="form-control" placeholder="{{ __('Email...') }}" value="{{ old('email', 'admin@uniandes.edu.co') }}" required>
              </div>
              @if ($errors->has('email'))
      ...
      <div class="row">
        <div class="col-6">
            @if (Route::has('password.request'))
                <a href="{{ route('password.request') }}" class="btn btn-secondary btn-xs">
                    <small>{{ __('¿Olvidó la contraseña?') }}</small>
                </a>
            @endif
        </div>
        <div class="col-6 text-right">
            <a href="{{ route('register') }}" class="btn btn-secondary btn-xs">
